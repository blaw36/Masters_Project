...
<!-- ```{r} -->
<!-- waveqtl_hmt_geno11$beta_dataS <- -waveqtl_hmt_geno11$beta_dataS * 100000000 -->
<!-- seq_sum <- seq_sum*1000 -->
<!-- ``` -->
...
1. Convert the mean effect into a proportion of baseline vs with effect (note that the effect can go both ways). We convert an additive effect (as per modelling assumptions) into a multiplicative ratio by:
\begin{align}
...
- currently assuming effect size ratio = 1 everywhere, outside the effect window
- $c_t$ is 0 at some bases -- currently just adding 1 to all $c_t$ to make sure we don't divide by 0.
...
effect_ratio <- 1+(70*waveqtl_hmt_geno11$beta_dataS/(seq_sum + 1))
...
plot(1,1,type="n"
     ...
     , xlim=c(1, 1024)
     , main ="Simulation - p1 (null) vs p2 (alt) parameters - before effect window"
     , axes=FALSE)
axis(2)
...
- Sequencing sum counts aren't always whole numbers. Draw from ceiling(sum) for now.
```{r}
set.seed(6)
# Null
...
for(i in 1:1024){
  null_data_50[,i] <- rmutil::rbetabinom(n = 70, size = ceiling(seq_sum[i])
