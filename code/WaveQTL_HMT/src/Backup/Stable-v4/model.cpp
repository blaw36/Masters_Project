...
// //// !!!!!!!! END BL DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!! ////
  ...
  vector<double> pi_1_list; // log of a prior prob for each phenotype being associated with genotype
  vector<double> pi_0_list;

  //--- BL_HMT start ---//
  // log of a prior joint (phenotype-parent) prob for that phenotype being associated with genotype
  // store all 4 combinations so we can work in logs
  vector<double> eps_11_list;
  vector<double> eps_01_list;
  vector<double> eps_10_list;
  vector<double> eps_00_list;
  //--- BL_HMT end ---//
  ...
  // set group end positions
  // set number of phenotypes in each group
  // This is used in the HMT algorithm for tying
  ...
  //--- BL_HMT start ---//
  // set tree-level positions, for backward-forward
  // algorithm. This is identical to the default group
  // allocation in WaveQTL as it follows the wavelet's 
  // scale structure, and hence, the tree 'level' 
  // structure.

  vector<int> tree_lvl_start;
  int num_tree_lvls = log2(nPH) + 1;

  tree_lvl_start.resize(0);
  for(int i = 0; i < num_tree_lvls; i++){
    if(i == 0){
      // First element is 1
      tree_lvl_start.push_back(1);
    }else{
      // Remaining elements are 2^{i - 1} + 1
      tree_lvl_start.push_back(pow(2,i - 1) + 1);
    }
  }

  vector<int> tree_lvl_end;
  vector<int> nPH_tree_lvl;

  tree_lvl_end.resize(0);
  nPH_tree_lvl.resize(0);
  for(int i = 1; i < num_tree_lvls; i++){
    tree_lvl_end.push_back(tree_lvl_start[i] - 1);
    nPH_tree_lvl.push_back(tree_lvl_start[i] - tree_lvl_start[i-1]);
  }
  tree_lvl_end.push_back(nPH);
  nPH_tree_lvl.push_back(nPH - tree_lvl_end[num_tree_lvls-2]);
...
// //// !!!!!!!! END BL DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!! ////
  ...
  // fstream outfile_eps_00; 
  // string sfn_eps_00("output/");
  // sfn_eps_00.append(fnOutput);
  // sfn_eps_00.append(".fph.eps_00.txt");
  // outfile_eps_00.open(sfn_eps_00.c_str(), ios::out);
  // if(!outfile_eps_00.is_open()) {
  //   cout << "can't open file ... " << endl;  
  //   exit(0); 
  // }
  //--- BL_HMT end ---//
...
// //// !!!!!!!! END BL DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!! ////
  ...
  // Alpha and beta quantities
    ...
    gsl_vector * pp_1 = gsl_vector_alloc(nPH);
    gsl_vector * pp_0 = gsl_vector_alloc(nPH);
    ...
    gsl_vector * pp_joint_00 = gsl_vector_alloc(nPH);
  ...
  // Vector of epsilon quantities
    ...
    gsl_vector * eps_00_vect = gsl_vector_alloc(nPH);
  ...
  // Define some variables
    ...
    double denom;
    double pp_sl_1, pp_sl_0;
    double pp_psl_1, pp_psl_0;
    double pp_joint_sl_11, pp_joint_sl_10, pp_joint_sl_01, pp_joint_sl_00;

    double diff;

  // Up step - don't do this step at coarsest scale.
    for(int gi = (num_tree_lvls - 1); gi > 0; gi--){

      int numP = nPH_tree_lvl[gi];
      int st = tree_lvl_start[gi] - 1;
      int parent_st = tree_lvl_start[gi - 1] - 1;

    // Step 0 at finest scale only (initialisation).
      if(gi == (num_tree_lvls - 1)){
        for(int i = 0; i < numP; i++){

          logBF = gsl_vector_get(logBFs, st+i);

          gsl_vector_set(beta_sl_1, st + i, logBF);
          gsl_vector_set(beta_sl_0, st + i, 0); // log(1) = 0
        }
      }
...
// //// !!!!!!!! END BL DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!! ////        
      ...
      // Get other child node of the same parent
      // If even-numbered, other child is on the right, else on the left
        if(i % 2 == 0){
        ...
        b_sl_psl_1 = sumlog(b_sl_1 + eps_11, b_sl_0 + eps_01);
        b_sl_psl_0 = sumlog(b_sl_0 + eps_00, b_sl_1 + eps_10);
// !!!! There's actually some duplication here: this below calculation
// will happen twice - at the adjacent node, the b_sl_psl above will be recalc'd
        b_sl_psl_adj_1 = sumlog(b_sl_adj_1 + eps_11, b_sl_adj_0 + eps_01);
        b_sl_psl_adj_0 = sumlog(b_sl_adj_0 + eps_00, b_sl_adj_1 + eps_10);

        b_psl_1 = logParentBF + b_sl_psl_1 + b_sl_psl_adj_1;
        b_psl_0 = b_sl_psl_0 + b_sl_psl_adj_0;
...
// //// !!!!!!!! END BL DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!! ////    
        ...
        // !!!! At the 1st (not 0th) group, we are at the root
        // of the tree. Hence, the 'parent' is the scaling coefficient,
        // which isn't part of the tree. So set this value to 0 for now.
  ...
  // Start at coarsest scale, then go until finest.
    for(int gi = 0; gi < num_tree_lvls; gi++){

      int numP = nPH_tree_lvl[gi];
      int st = tree_lvl_start[gi] - 1;

    // !!!! Don't know what to do at the moment for top (scaling) coefficient - not part of tree.
    // Set to 0 for now.
      if(gi == 0){
        gsl_vector_set(alpha_sl_1, st, 0);
        gsl_vector_set(alpha_sl_0, st, 0);
...
// //// !!!!!!!! END BL DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!! ////
          ...
          a_sl_1 = sumlog(eps_11 + a_psl_1 + b_psl_no_sl_1, eps_10 + a_psl_0 + b_psl_no_sl_0);
          a_sl_0 = sumlog(eps_01 + a_psl_1 + b_psl_no_sl_1, eps_00 + a_psl_0 + b_psl_no_sl_0);
...
// //// !!!!!!!! END BL DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!! ////    
  ...
  // ---- Calculate the E-step quantities (posterior probabilities) ---- //
    for(int wc = 0; wc < nPH; wc++){
      ...
      // Calculating the marginal of gamma_sl, and
      // posterior joint of gamma_sl, gamma_psl

      if(wc == 0){

        // !!!! Neither coefficients are defined for first coefficient,
        // as it's a scaling coefficient. Set to 0 for now.

        gsl_vector_set(pp_1, wc, 0);
        gsl_vector_set(pp_0, wc, 0);
        gsl_vector_set(pp_joint_11, wc, 0);
        gsl_vector_set(pp_joint_10, wc, 0);
        gsl_vector_set(pp_joint_01, wc, 0);
        gsl_vector_set(pp_joint_00, wc, 0);
        ...
        denom = sumlog(b_sl_1 + a_sl_1, b_sl_0 + a_sl_0);
        pp_sl_1 = b_sl_1 + a_sl_1 - denom;
        pp_sl_0 = b_sl_0 + a_sl_0 - denom;
        gsl_vector_set(pp_1, wc, pp_sl_1);
        gsl_vector_set(pp_0, wc, pp_sl_0);

        // !!!! Joint marginal NOT DEFINED for second coefficient either,
        // as this location is the top of the tree (no parent)
        // Set to 0 for now.
        gsl_vector_set(pp_joint_11, wc, 0);
        gsl_vector_set(pp_joint_10, wc, 0);
        gsl_vector_set(pp_joint_01, wc, 0);
        ...
        pp_joint_sl_00 = b_sl_0 + eps_00 + a_psl_0 + b_psl_no_sl_0 - denom;
...
// //// !!!!!!!! END BL DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!! ////            
        ...
        gsl_vector_set(pp_joint_00, wc, pp_joint_sl_00);
  ...
  // states of gamma_11.
    ...
    N_obllikli = sumlog(b_11_1 + pi_1, b_11_0 + pi_0);
    O_obllikli = N_obllikli;
...
////// !!!! WITH TYING ///////////////
        // Always skip the 'zero-th' group for tying;
        // Zero-th group only contains coefficient '0',
        // the scaling coefficient.

        // Notes on the section below:
        // Due to how our 'sumlog' function operates, if we need 
        // to average over more than 2 posteriors to estimate
        // our parameter, we need to do an initial 'sumlog' over
        // the first two elements, then iteratively add every extra
        // element as we go. This is because sumlog only
        // works on two input arguments. It's a little cumbersome,
        // but that's how we're going to continue keeping
        // all our quantities in 'log' form to reduce
        // risk of underflow.

        // Hence we split both operations up into separate cases:
          // - Only 1 probability involved in estimation
          // - >=2 probabilities involved in estimation

        // Estimating Pi parameter - only 1 pi param required.
				// Pi is never really tied, there is only ever one 'pi' parameter
				// for us, which is at the root of the tree. Hence, there is no
				// 'tying' of pi parameters. It's always only made up of the root node's
				// respective probabilities.

				// !!!! No Pi 'tying' !!!! ////
				int st = group_start[1] - 1;
				pi_1 = gsl_vector_get(pp_1,st);
				pi_0 = gsl_vector_get(pp_0,st);
        ...
        //     double pi_1_next = gsl_vector_get(pp_1, st + 1 + i);
        //     double pi_0_next = gsl_vector_get(pp_0, st + 1 + i);

        //     pi_1 = sumlog(pi_1, pi_1_next);
        //     pi_0 = sumlog(pi_0, pi_0_next);
        //   }
        ...
        // Position 0 is scaling coefficient.
        // Position 1 is head of tree - no parent.
        ...
        for(int wc = 0; wc < 2; wc++){
          ...
          gsl_vector_set(eps_00_vect, wc, 0);
        }

        // For the remaining, do the tying procedure.

        // For each group (starting at group 1 - the 0-th group is
        ...
        for(int gi = 1; gi < numG; gi++){
          ...
          if(numP == 1){
            ...
            eps_11 = pp_joint_sl_11 - pp_psl_1;
            eps_01 = pp_joint_sl_01 - pp_psl_1;
            ...
            // std::cout << "pp_joint_sl_10:" << pp_joint_sl_10 << "\n";       
            // std::cout << "pp_joint_sl_01:" << pp_joint_sl_01 << "\n";       
            // std::cout << "pp_joint_sl_00:" << pp_joint_sl_00 << "\n";       
            // std::cout << "pp_psl_1:" << pp_psl_1 << "\n";       
            // std::cout << "pp_psl_0:" << pp_psl_0 << "\n";       
            // //// !!!!!!!! END BL DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!! ////  
            ...
            for(int i = 0; i < extra_ind; i++){
              ...
              double pp_joint_sl_01_next = gsl_vector_get(pp_joint_01, next_ind);
              double pp_joint_sl_00_next = gsl_vector_get(pp_joint_00, next_ind);

              pp_joint_sl_11 = sumlog(pp_joint_sl_11, pp_joint_sl_11_next);
              pp_joint_sl_10 = sumlog(pp_joint_sl_10, pp_joint_sl_10_next);
              pp_joint_sl_01 = sumlog(pp_joint_sl_01, pp_joint_sl_01_next);
              pp_joint_sl_00 = sumlog(pp_joint_sl_00, pp_joint_sl_00_next);

              int parent_wc = next_ind / 2; // integer division
              double pp_psl_1_next = gsl_vector_get(pp_1, parent_wc);
              double pp_psl_0_next = gsl_vector_get(pp_0, parent_wc);

              pp_psl_1 = sumlog(pp_psl_1, pp_psl_1_next);
              pp_psl_0 = sumlog(pp_psl_0, pp_psl_0_next);

              // //// !!!!!!!! BL DEBUGGING - REMOVE LATER!!! !!!!!!!! ////
              // std::cout << "next_ind:" << next_ind << "\n";
              // std::cout << "parent_wc:" << parent_wc << "\n";
              // std::cout << "pp_joint_sl_11_next:" << pp_joint_sl_11_next << "\n";       
              // std::cout << "pp_joint_sl_10_next:" << pp_joint_sl_10_next << "\n";       
              // std::cout << "pp_joint_sl_01_next:" << pp_joint_sl_01_next << "\n";       
              // std::cout << "pp_joint_sl_00_next:" << pp_joint_sl_00_next << "\n";       
              // std::cout << "pp_psl_1_next:" << pp_psl_1_next << "\n";       
              // std::cout << "pp_psl_0_next:" << pp_psl_0_next << "\n";       
              // std::cout << "pp_joint_sl_11:" << pp_joint_sl_11 << "\n";       
              // std::cout << "pp_joint_sl_10:" << pp_joint_sl_10 << "\n";       
              // std::cout << "pp_joint_sl_01:" << pp_joint_sl_01 << "\n";       
              // std::cout << "pp_joint_sl_00:" << pp_joint_sl_00 << "\n";       
...
//       pi_1 = gsl_vector_get(pp_1, 1);
//       pi_0 = gsl_vector_get(pp_0, 1);
...
//           pp_joint_sl_00 = gsl_vector_get(pp_joint_00, wc);
...
//           gsl_vector_set(eps_00_vect, wc, eps_00);
...
// ////// !!!! END NO TYING ///////////////      
      ...
      // Does this remain the same with each iteration?
        if(gi == (num_tree_lvls - 1)){
          for(int i = 0; i < numP; i++){
            ...
            gsl_vector_set(beta_sl_1, st + i, logBF);
            gsl_vector_set(beta_sl_0, st + i, 0);
          }
        }
        ...
        for(int i = 0; i < numP; i++){
          ...
          eps_00 = gsl_vector_get(eps_00_vect, st + i);
        ...
        // If even, grab one to the right, else one to the left
          if(i % 2 == 0){
            ...
            b_sl_adj_0 = gsl_vector_get(beta_sl_0, st + i + 1);
	          ...
	          eps_adj_01 = gsl_vector_get(eps_01_vect, st + i + 1);
	          eps_adj_10 = gsl_vector_get(eps_10_vect, st + i + 1);
	          eps_adj_00 = gsl_vector_get(eps_00_vect, st + i + 1);
          }else{
            b_sl_adj_1 = gsl_vector_get(beta_sl_1, st + i - 1);
            b_sl_adj_0 = gsl_vector_get(beta_sl_0, st + i - 1);
	          eps_adj_11 = gsl_vector_get(eps_11_vect, st + i - 1);
	...
	// values in one big chunk, rather than location-wise.
	        b_sl_psl_adj_1 = sumlog(b_sl_adj_1 + eps_adj_11, b_sl_adj_0 + eps_adj_01);
	        b_sl_psl_adj_0 = sumlog(b_sl_adj_0 + eps_adj_00, b_sl_adj_1 + eps_adj_10);
    ...
    // Start at coarsest scale, then go until finest.
      ...
      for(int wc = 0; wc < nPH; wc++){
        ...
        if(wc == 0){
          ...
          eps_00 = gsl_vector_get(eps_00_vect, wc);
...
//     //// !!!!!!!! END BL DEBUGGING !!!!!!!!!!!!!!!!!!!!!!!!! ////
  ...
  // Only one of these values for the whole algorithm
    logLR = N_obllikli;
    pi_1_list.push_back(pi_1);
    pi_0_list.push_back(pi_0);
    logLR_list.push_back(logLR);
