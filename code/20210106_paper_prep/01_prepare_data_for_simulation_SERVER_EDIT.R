...
print(paste(Sys.time(),paste("Command line arg:", args[1]), sep = " - "))
...
path = paste0(locus.path, "chr", args[1], ".loc")
...
for(site in 1:num_rows){
  ...
  # Ideally, each row of this file would have a corresponding maf_chr file
  # Max position # for each chromosome corresponds with # of rows in each file
  # So we know that the row # corresponds to a position in a chromosome
  # Unfortunately, some are missing. # rows > # of files
  # So some files we'll just note that their files are missing and skip processing the rows

  geno_file_exists = file.exists(geno.info.path)
  ...
  if(geno_file_exists){
    ...
    write.table(res$DNase.dat, path.output, row.names = FALSE, col.names = FALSE, quote = FALSE)

  }else{
    # If file doesn't exist

    print(paste(Sys.time(), paste("File doesn't exist for site:",site), sep = " - "))
